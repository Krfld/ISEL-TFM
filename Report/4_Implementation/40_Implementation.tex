...
\subsection{Event States}
\label{subsec:event_states}
...
After the \textit{No refund} date, we will prevent the users to call the refund method, which will \textit{burn} the NFTs, essentially removing them from the user and making them available again.
...
The other good thing for the organizer is in case the event is expected to be sold out. Since the users will get some money back, they will have a reason to refund their tickets, if they cannot attend the event anymore, making them available again for other users to buy at the original price, making the organizer a higher profit.
After this deadline, the only thing to allow is for users to resell their tickets in our marketplace, which will allow the user to sell them at a higher price than the refund (but never higher than the original, of course).
...
The \textit{End} date tells when the event is over, unlocking all the ticket operations to outside the system. So users can simply keep the tickets as a souvenir or sell them in any marketplace, without any restrictions on the tickets, including the elimination of the resell price cap.

% Like the Figure \ref{fig:event_uml} shows, we added the methods for the users to handle the tickets and the organizer/admins to manage the event.
